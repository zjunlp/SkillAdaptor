...
def build_validation_audit(*, skill_id: str, created_from: Optional[str], adopted: bool, adopt_result: Any, scoped_result: Optional[Any]=None, detail: str='') -> Dict[str, Any]:
    ...
    revised = getattr(adopt_result, 'revised_metrics', None) or {}
    frozen = revised.get('retrieval_frozen_tasks')
    return {
        'skill_id': skill_id,
        'created_from': created_from,
        'decision': 'ADOPTED' if adopted else 'REJECTED',
        'detail': detail,
        'full_q': _snap(adopt_result),
        'source': _snap(scoped_result),
        'adoption_scope': list(revised.get('adoption_scope') or []),
        'frozen_regression': bool(revised.get('frozen_regression')),
        'retrieval_frozen_tasks': frozen,
    }
