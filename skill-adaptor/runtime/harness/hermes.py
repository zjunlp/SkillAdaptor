...
from runtime.skill_export import build_frontmatter, sync_workspace_skills_to_hermes
...
def ensure_hermes_skill_markdown(skill_text: str, *, skill_id: str = EVOLVED_SKILL_DIR) -> str:
    """Hermes skills require YAML frontmatter (name + description) on SKILL.md."""
...
class HermesHarness:
    ...
    def _hermes_evolved_path(self) -> Path:
        return (
            _hermes_home()
            / 'skills'
    ...
    def _workspace_evolved_path(self) -> Path:
        return (
            self.project_root
            / '.hermes'
            / 'skills'
    ...
    def inject_skill_text(self, skill_text: str, *, benchmark_root: Path, task_id: Optional[str] = None) -> None:
        if not skill_text:
            return
        md = ensure_hermes_skill_markdown(skill_text)
        ...
        for target in (self._hermes_evolved_path(), self._workspace_evolved_path()):
    ...
    def clear_skill_injection(self, *, benchmark_root: Path, task_id: Optional[str] = None) -> None:
        for skill_file in (
            ...
            self._hermes_evolved_path(),
            self._workspace_evolved_path(),
        ):
