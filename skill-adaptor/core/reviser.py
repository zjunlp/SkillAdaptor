...
from .types import Skill, LocalizedFault, SkillAttribution
...
class Reviser:
    ...
    def revise(self, skill: Skill, fault: LocalizedFault, attribution: SkillAttribution, iterative: bool=False, rejection_summaries: Optional[List[str]]=None) -> Optional[Skill]:
        if self.llm_client is None:
            raise RuntimeError('LLM client not configured. Reviser requires an LLM client.')
        self.record_fault(skill.id, fault)
        if iterative:
            return self._revise_iterative(skill, fault, attribution, rejection_summaries)
        return self._revise_with_llm(skill, fault, attribution, rejection_summaries)

    def _revise_iterative(self, skill: Skill, fault: LocalizedFault, attribution: SkillAttribution, rejection_summaries: Optional[List[str]]=None) -> Optional[Skill]:
        current_skill = skill
        revised_skill = None
        for iteration in range(self.max_iterations):
            result = self._revise_with_llm(current_skill, fault, attribution, rejection_summaries)
            if result is None:
                break
    ...
    def _revise_with_llm(self, skill: Skill, fault: LocalizedFault, attribution: SkillAttribution, rejection_summaries: Optional[List[str]]=None) -> Optional[Skill]:
        history = self._fault_history.get(skill.id, [])
        prompt = self._build_revision_prompt(skill, fault, attribution, history, rejection_summaries=rejection_summaries)
        try:
            response = self.llm_client.chat.completions.create(model=self.model_name, messages=[{'role': 'user', 'content': prompt}], temperature=0.2)
        ...
        if revision_data.get('skill_profile') and isinstance(revision_data['skill_profile'], dict):
            return self._apply_skill_profile_revision(skill, revision_data, fault)
        self._validate_revision_data(revision_data)
        if revision_data.get('revision_type') == 'none':
            return None
        return self._apply_structured_revision(skill, revision_data, fault)
    ...
    def _build_revision_prompt(self, skill: Skill, fault: LocalizedFault, attribution: Any, history: Optional[List], rejection_summaries: Optional[List[str]]=None) -> str:
        history_note = ''
        pattern_warning = ''
