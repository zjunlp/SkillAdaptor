...
from .types import LocalizedFault, SkillAttribution, Skill
...
class Linker:
    ...
    def attribute(self, fault: LocalizedFault, skill_bank: Dict[str, Skill], llm_client: Any, skill_matcher: Optional[Any]=None) -> List[SkillAttribution]:
        # Paper: Link(q, τ, t*, S_q) is LLM over candidate skills. skill_missing with
        # an empty bank → nothing to attribute. Otherwise always call the LLM (do not
        # short-circuit with a rule that skips Link).
        if not skill_bank:
            return []
    ...
    def _attribute_with_llm(self, fault: LocalizedFault, skill_bank: Dict[str, Skill], llm_client: Any, skill_matcher: Optional[Any]=None) -> List[SkillAttribution]:
        ...
        for item in raw_items:
            ...
            if 'weight' not in item:
                # Do not invent a soft default weight (would pollute revise gating).
                continue
            try:
                weight = float(item['weight'])
            except (TypeError, ValueError) as exc:
                raise ValueError(f'Linker attribution weight must be numeric, got {item.get("weight")!r}') from exc
        ...
        # Paper path: Linker weights come from the LLM only. Do not fabricate
        # attributions when the model returns empty / low-confidence results.
        return attributions
    ...
    def _get_relevant_skills(self, fault: LocalizedFault, skill_bank: Dict[str, Skill], skill_matcher: Optional[Any]=None) -> Dict[str, Skill]:
        ...
        if fault.skills_at_fault:
            ...
            if found:
                return found
        # Prefer embedding top-k over S_q / bank whenever matcher is available
        # (not only skill_wrong) so large banks never look "empty" to the LLM.
        if skill_matcher is not None:
            query = (
                f'{fault.task_id} {fault.observation[:400]} '
                f'{fault.wrong_action[:120]} {fault.improvement_principle[:200]}'
            )
            matches = skill_matcher.match_skills_to_task(skill_bank, query, top_k=min(10, len(skill_bank)))
            if matches:
                return {skill.id: skill for skill, _ in matches}
        # Never return {} when bank is non-empty — that forces a false
        # "No skills were active" prompt and breaks Link/Revise.
        items = list(skill_bank.items())[:10]
        return dict(items)
    ...
    def _build_attribution_prompt(self, fault: LocalizedFault, skill_bank: Dict[str, Skill]) -> str:
        if skill_bank:
            ...
            skills_text = 'No candidate skills available in the bank for this fault.'
    ...
    def attributions_for_repair(
        ...
        """High-confidence LLM attributions only (no fabricated repair weights)."""
        del fault, skill_bank  # candidates already filtered by Linker LLM
        return self.filter_high_confidence(attributions, threshold)
